...
public class Eat implements Action {
    ...
    public ArrayList<Point> get_move_place(DBhelper dbHelp, int x, int y, int fig_id) {
        ...
        try{
            ...
            dbHelp.opendatabase();
            Cursor rs = dbHelp.myDataBase.rawQuery("Select * from eat where id_figure = "+figure.id,null);
            while (rs.moveToNext()) {
                move_x = rs.getInt(rs.getColumnIndex("eat_x"));
                move_y = rs.getInt(rs.getColumnIndex("eat_y"));
                sort = rs.getInt(rs.getColumnIndex("sort"));
                movement_id = rs.getInt(rs.getColumnIndex("eatment_id"));
                if (Desc.is_in_desc(x+move_x, y+move_y)) {
                    if (if_free_cell( dbHelp,x,y,x+move_x,y+move_y,sort,movement_id,fig_id)) {
